...
class BinaryTree(object):
    ...
    def deletion(self, root, key):   # pass in the root and Key, which is the Node to be deleted
        ...
        while(len(queue)): 
            ...
            print("Temporary Value: ", temp.value)
        ...
        if key_node:  
            ...
            print("The Key Node is: ", key_node.value)
            x = temp.value    
            
            print("Temp Value", x)
                
            self.delete_deepest(root,temp)      # call delete Deepest function, pass in our root and the temp value
                                                # NOTE: in order to call method from the same class, we need the self keyword
                
            key_node.value = x  # after we deleted the deepest value, we will set our Key Node = to the deepest value
